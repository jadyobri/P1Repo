...
def path_to(source_point, destination_point, source_box, destination_box, forward_path, backward_path, middle_box, middle_point, detail_points):
    ...
    while current_box is not None and current_box != source_box:
        if current_box not in forward_path:
            print(f"Error: Incomplete forward path from {current_box}")
    ...
    while current_box is not None and current_box != destination_box:
        if current_box not in backward_path:
            print(f"Error: Incomplete backward path from {current_box}")
            return []  # Path reconstruction failed
...
def bidirectional_a_star_search(source_point, destination_point, source_box, destination_box, adjacencies, visited_boxes, detail_points):
    ...
    forward_costs[source_box] = (0, forward_h)
